...
export const api = {
  fetchFullState: async () => {
    try {
      ...
      if (intError) console.error('Error fetching interviews:', JSON.stringify(intError, null, 2));
      ...
      if (blockError) console.error('Error fetching blocks:', JSON.stringify(blockError, null, 2));
      ...
      if (notifError) console.error('Error fetching notifications:', JSON.stringify(notifError, null, 2));
      const notifications = (notifData || []).map(fromDbNotif);

      return { users, interviews, blockedSlots, notifications };

    } catch (e: any) {
      console.error("Supabase Fetch Critical Error:", e?.message || JSON.stringify(e));
